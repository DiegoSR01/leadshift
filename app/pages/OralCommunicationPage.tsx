...
export function OralCommunicationPage() {
  const [selectedExercise, setSelectedExercise] = useState<null | typeof exercises[0]>(null);
  ...
  const mockScore = selectedExercise ? {
    total: 83,
    breakdown: selectedExercise.criteria.map((c) => ({
      ...c,
  ...
  if (phase === 'result' && selectedExercise && mockScore) {
    return (
      <div className="min-h-screen bg-slate-50">
        <div className="bg-white border-b border-slate-200 px-8 py-4">
        ...
        <div className="px-8 py-6 max-w-3xl mx-auto space-y-5">
          ...
          <div className="bg-gradient-to-r from-violet-600 to-violet-900 rounded-2xl p-7 text-center text-white">
            <div className="text-6xl font-extrabold mb-2">{mockScore.total}</div>
            <div className="text-violet-200 mb-4">puntos sobre 100</div>
            <div className="flex items-center justify-center gap-2">
              {[...Array(5)].map((_, i) => (
                <Star key={i} className={`w-6 h-6 ${i < Math.round(mockScore.total / 20) ? 'text-yellow-400 fill-yellow-400' : 'text-violet-400'}`} />
              ))}
            </div>
          </div>
